...
function plotflow(A, flow; darrow=false, offset=.05, alpha=.1, ncluster=3)
    ...
    res = spectralCluster(laplacian, ncluster)


    # Coloring nodes
    membership = res.assignments
    nodecolor = distinguishable_colors(ncluster, colorant"blue")
    # membership color
    ...
    title("cluster")
    subplot(212)
    scatter(posx, posy, s=150, c=getshape(pb), zorder=2)
    title("house profile")
    axis("off")
